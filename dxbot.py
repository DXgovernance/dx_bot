...
mainnet = {
    "net": "MAINNET",
    "emoji": ":eth:",
    "rpc": os.getenv("ENDPOINT_MAINNET"),
    "vms": [
...
arbitrum = {
    "net": "ARBITRUM",
    "emoji": "",
    "rpc": os.getenv("ENDPOINT_ARBITRUM"),
...
def fetch_proposals(net):
    ...
    for vm in net["vms"]:
        ...
        # Filter by avatar
        _proposals = [
            p for p in _proposals if p["args"]["_organization"] == net["avatar"]
        ]
        # Filter by state
        _proposals = [
            p
            for p in _proposals
            if vm.functions.state("0x" + p["args"]["_proposalId"].hex()).call()
            in [4, 5, 6]
        ]
        # Get scheme for each proposal
        for p in _proposals:
            _proposal_state = vm.functions.proposals(
                "0x" + p["args"]["_proposalId"].hex()
            ).call()
...
def parse_proposals(proposals, net):
    ...
    cs = w3.eth.contract(
        ...
        abi=json.load(open("abis/contributorScheme.json")),
    )
    mc = w3.eth.contract(
        address="0x8E900Cf9BD655e34bb610f0Ef365D8d476fD7337",
        abi=json.load(open("abis/multiCall.json")),
    )
    ens = w3.eth.contract(
        address="0x8E900Cf9BD655e34bb610f0Ef365D8d476fD7337",
        abi=json.load(open("abis/ensScheme.json")),
    )
    scheme_register = w3.eth.contract(
        address="0x8E900Cf9BD655e34bb610f0Ef365D8d476fD7337",
        abi=json.load(open("abis/schemeRegister.json")),
    )
    ...
    for _p in proposals:
        ...
        if _p["scheme"] in contributorSchemes:
            ...
            logs = [
                l
                for l in logs
                if "0x" + l["args"]["_proposalId"].hex() == _p["proposalId"]
            ]
            p = logs[0]["args"]
...
def get_title(proposal_hash):
    ...
    try:
        ...
        data = {"hash": proposal_hash, "title": title, "text": text}
